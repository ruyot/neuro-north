...
from ssvep.trca_model import count_blocks, cross_validate, load_training_data, resolve_data_dir
...
def main() -> None:
    ...
    data_dir = resolve_data_dir(args.session)
    session = os.path.basename(os.path.normpath(data_dir))
    n_blocks = count_blocks(data_dir)
    print(f"Session: {session} ({n_blocks} complete blocks)\n")
    ...
    report(eeg, labels, session, plot_path)
