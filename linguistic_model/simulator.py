...
def _load_bigram_counts(path: Path) -> Dict[Tuple[str, str], float]:
    ...
    if not path.exists():
        # The source branch did not commit data/bigrams.tsv. With no corpus,
        # the lightweight engine is a unigram fallback; causal scoring still
        # uses full context. Do not manufacture context counts.
        return counts
